...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/30 16:37:48 by ctanton           #+#    #+#             */
/*   Updated: 2021/03/30 16:37:58 by ctanton          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../cub3d.h"

void	ft_parse_f2_first(char *tmp, t_all *all)
{
	char	*dst;
	char	**tmp2;
	int		i;
	...
	if (all->flag2->flag_for_f == 0)
		all->flag->color_r = ft_atoi(dst);
	else if (all->flag2->flag_for_f == 1)
		all->flag->color_g = ft_atoi(dst);
	else if (all->flag2->flag_for_f == 2)
		all->flag->color_b = ft_atoi(dst);
	all->flag2->flag_for_f++;
	free(dst);
	ft_free(tmp2);
}

void	ft_parse_f2(char *tmp, t_all *all)
{
	char	*dst;
	char	**tmp2;
	int		j;
...
void	ft_parse_f3(t_all *all)
{
	if (all->flag->color_r > 255 || all->flag->color_g > 255 || \
			all->flag->color_b > 255)
		ft_error(8, all);
	all->texture->floor = create_rgb(all->flag->color_r, \
...
void	ft_parse_f(char *identifiers, t_all *all)
{
	...
	if (tmp[0] == NULL || tmp[1] == NULL || tmp[2] == NULL || tmp[3] != NULL)
		ft_error(8, all);
	ft_parse_f2_first(tmp[0], all);
	ft_parse_f2(tmp[1], all);
	ft_parse_f2(tmp[2], all);
	ft_parse_f3(all);
	i = 0;
	while (tmp[i] != NULL)
	{
